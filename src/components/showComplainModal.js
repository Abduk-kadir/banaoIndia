...
const ShowComplain = ({ isModalVisible, toggleModal, data }) => {
  ...
  const success = useSelector(
    (state) => state?.complaints?.cComplaint?.success,
  ); // For Send OTP input
  ...
  const verifystatus = useSelector(
    (state) => state?.emails?.verifyData?.success,
  );
  const completeComplainSuccess = useSelector(
    (state) => state?.complaints?.complaint?.success,
  );
  const completeComplainLoading = useSelector(
    (state) => state?.complaints?.loading,
  );
  const cancelComplainSuccess = useSelector(
    (state) => state?.complaints?.cancelComplaint?.success,
  );
  const cancelComplainLoading = useSelector(
    (state) => state?.complaints?.cancelLoading,
  );
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.inputSection}>
              ...
              <Text style={styles.complainText}>
                {data?.complainText || "No message provided"}
              </Text>
            </View>
            ...
            {completeComplainLoading || cancelComplainLoading ? (
              <Loader />
            ) : (
              <TouchableOpacity
